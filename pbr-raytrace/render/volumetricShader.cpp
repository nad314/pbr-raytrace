...
namespace core {
	...
	const vec4s VolumetricShader::getColor(const Ray& ray, const float& d, const vec4s& normal, const vec4s& color, const PBVH& bvh, std::pair<int, float>* stack, int* priority, int rek) const {
		...
		const vec4s L = mcrotate(normal, material.roughness).normalized3d();
		const vec4s V = (lightRay.sr1*vec4s(-1.0f)).normalized3d();
		const vec4s H = (L + V).normalized3d();
		const vec4s N = normal.normalized3d();
		const vec4s newDir = mcrotate(normal, 1.0f).normalized3d();
		...
		const vec4s fresnel = fresnel_factor(specular, ndv.m.m128_f32[0]).w1();
		...
		const vec4s iblspec = (cooktorrance_specular(ndl.m.m128_f32[0], ndv.m.m128_f32[0], ndh.m.m128_f32[0], fresnel, material.roughness) + fresnel).min(1.0f);
		...
		lray = lightRay;
		lray.d = -100.0f;
		//const vec4s nndl = mix(vec4s(1.0f), N.dot3(newDir), material.roughness);
		if (bvh.findFirst(lray, stack, priority, false) < 0.0f)
