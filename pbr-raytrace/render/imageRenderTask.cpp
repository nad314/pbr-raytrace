...
*/
		...
		for (int gy = 0; gy < h; gy += square) {
			/*if (gy > bq.y || (gy + square) < bp.y)
				continue;*/
			for (int gx = square*worker.threadNumber; gx < w; gx += square*worker.threadCount) {
			//task.onStartNode(pview, vec2i(gx / square, gy / square), vec2i((int)std::ceil((float)(img.width)/ square), (int)std::ceil((float)(img.height)/ square)), square);
				...
				const int my = std::min(gy + square, h);
					...
					for (int i = gy; i < my; ++i) {
						for (int j = gx; j < mx; ++j) {
							...
							const vec4s frag = simdFrag * vec4s(255.0f) / vec4s((float)ms);
				...
				/*
			if (worker.threadNumber == 0)
				task.onEndNode(pview, vec2i(gx / square, gy / square), vec2i((int)std::ceil((float)(img.width) / square), (int)std::ceil((float)(img.height) / square)), square);
				*/
			}
		}
		delete[] priority;
